...
This time we will use nn.Sequential to create the network which requires setting 'init_hidden=True' for every
layer and 'output=True' for the last layer. 
...
net = nn.Sequential(nn.Linear(num_inputs, num_hidden, bias=False),
                    RAF(frequency, beta, init_hidden=True),
                    nn.Linear(num_hidden, num_outputs, bias=False),
                    RAF(frequency, beta, init_hidden=True, output=True)
                   ).to(device)


# Define a function used for the forward pass when 'init_hidden=True'
def forward_pass(net, num_steps, data):
  ...
  for step in range(num_steps):
      _, V_out, spk_out = net(data[step])
      spk_rec.append(spk_out)
      V_rec.append(V_out)

  return torch.stack(spk_rec), torch.stack(V_rec)
...
def print_batch_accuracy(data, targets, train=False):
    output, _ = forward_pass(net, num_steps, data.view(num_steps, batch_size, -1))
...
for epoch in range(num_epochs):
    ...
    for data, targets in train_batch:
        ...
        # Forward pass
        net.train()
        spk_rec, V_rec = forward_pass(net, num_steps, spike_data.view(num_steps, batch_size, -1))
        ...
        with torch.no_grad():
            ...
            test_spk, test_V = forward_pass(net, num_steps, spike_test_data.view(num_steps, batch_size, -1))
    ...
    with torch.no_grad():
      ...
      for data, targets in test_loader:
        ...
        # Forward pass
        test_spk, _ = forward_pass(net, num_steps, spike_data.view(num_steps, batch_size, -1))
